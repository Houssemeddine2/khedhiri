...
export default function ActiveCallBar() {
  const { status, callType, peer, startedAt, muted, hangUp, toggleMute, callError, remoteStream } = useCall()
  const duration = useDuration(status === 'active' ? startedAt : null)
  const audioRef = useRef<HTMLAudioElement>(null)

  // Brancher le flux audio distant sur l'élément <audio> pour les appels vocaux
  useEffect(() => {
  ...
  return (
    <>
      {/* Élément audio caché — indispensable pour entendre l'interlocuteur en appel vocal */}
