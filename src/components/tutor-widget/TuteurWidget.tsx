...
export default function TuteurWidget({ prenom }: TuteurWidgetProps) {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: reply ?? 'Désolée…' }])
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      ...
      {!open && (
        <button
          ...
          aria-label="Ouvrir Nour, la tutrice"
        ...
        <div className="fixed bottom-5 right-5 z-40 w-80 max-h-[70vh] bg-white rounded-2xl shadow-2xl border border-sand-warm flex flex-col overflow-hidden">
          ...
          <div className="bg-olive px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="font-manrope font-semibold text-white text-sm">Nour</p>
                <p className="font-manrope text-white/70 text-xs">Ta tutrice</p>
              </div>
            </div>
          ...
          <div className="border-t border-sand-warm px-2 py-2 flex gap-2">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-olive flex items-center justify-center text-white disabled:opacity-40 hover:bg-olive/80 transition-colors"
            >
